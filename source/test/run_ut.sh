...
# Check if change number/revision is provided for RdkbGMock
if [ -n "$1" ]; then
    change_revision=$1
...
log "INFO" "Running make for parodusStart_gtest.bin..."
...
if [ -f "./source/test/parodusStart_gtest.bin" ]; then
    log "INFO" "Running parodusStart_gtest.bin"
    ./source/test/parodusStart_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "parodusStart_gtest.bin does not exist, cannot run tests"
    exit 1
fi
